...
class DioInterceptorLogger extends Interceptor {
  ...
  /// Print requests header [Options.headers]
  final bool requestHeader;

  /// Print requests data [Options.data]
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (var i = 0; i < lines; ++i) {
            final min = math.min<int>(i * linWidth + linWidth, msg.length);
            logPrint('║${_indent(_tabs)} '
                '${msg.substring(i * linWidth, min)}');
          }
        } else {
